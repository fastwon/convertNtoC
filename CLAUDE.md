...
- **Provider abstraction** (decided): the app supports **Claude by default + a Google Gemini free tier behind a "free mode" toggle**. Route everything through an `LLMProvider` interface in `app/llm/` (Claude impl + Gemini impl); never scatter `anthropic.Anthropic()`/Gemini calls across the codebase. Each impl loads its key from the keyring. Provider differences (prompt caching, structured output) are mapped *inside* each impl, not leaked to callers. The `claude-api` skill rule above governs the **Claude impl** specifically. Project memory is data-driven (global memory re-injected per episode), so the free provider degrades text quality, not the consistency mechanism — see `docs/DESIGN.md` §6.0.
